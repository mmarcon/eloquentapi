...
function describeResource(controller, basePath) {
    ...
    schema.resourcePath = controller.route.replace(/\(.*\)/, '{' + controller.name + '}').replace('?', '');

    api.path = controller.route.replace(/\(.*\)/, '{' + controller.name + '}').replace('?', '');
    ...
    methods.forEach(function (m) {
        ...
        operation.nickname = controller.nickname || controller.route.replace(/\//g, "").replace(/\(.*\)/, '{' + controller.name + '}');
...
function describeAPI(registry, basePath) {
    ...
    for (r in registry.routes) {
        if (registry.routes.hasOwnProperty(r)) {
            ...
            resources.apis.push({
                path: '/doc' + r.replace(/\(.*\)/, '{' + 'monkey' + '}').replace('?', ''),
